...
function verifyRegistryProjection(repositoryRoot: string, value: JsonValue): void {
  const source = JSON.parse(readFileSync(join(repositoryRoot, "contracts/registries/v1/registry-set.json"), "utf8")) as {
...
export function buildTypeScriptConformanceResult(repositoryRoot: string): ConformanceResult {
  ...
  const manifestPath = join(repositoryRoot, "contracts/fixtures/v1/manifest.json");
  ...
  const adapterSources = ["native-validator.ts", "strict-json.ts", "dp008-typescript-jcs-adapter.ts"]
