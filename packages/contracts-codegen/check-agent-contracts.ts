...
function buildManifest(): { manifest: JsonObject; text: string } {
  ...
  const relationships = [
    ...
    { id: "command-server-fields", rule: "intent-only-commands-reject-caller-owned-server-fields", fixtures: ["invalid-create-agent-run-request.caller-owned-run-id", "invalid-create-agent-run-request.caller-owned-status", "invalid-create-agent-run-request.caller-owned-workspace", "invalid-issue-apply-authorization-request.caller-owned-issuer", "invalid-issue-apply-authorization-request.caller-owned-expiry"], expected: "reject" },
