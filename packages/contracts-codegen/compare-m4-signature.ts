...
export function compareM4Signature(repositoryRoot: string, paths: string[]): object {
  if (paths.length !== 4) throw new Error(`expected four --result paths, got ${paths.length}`);
  const corpusBytes = readFileSync(join(repositoryRoot, "contracts/governance/m3/signature-cases.json"));
...
if (import.meta.main) {
  ...
  console.log(JSON.stringify(compareM4Signature(root, resultPaths()), null, 2));
}
