...
export function compareAgentConformance(repositoryRoot: string, resultPaths: string[]): object {
  if (resultPaths.length !== 2) throw new Error(`expected two --result paths (go, typescript), got ${resultPaths.length}`);
  const manifestBytes = readFileSync(join(repositoryRoot, "contracts/agent/fixtures/manifest.json"));
  ...
  for (const result of results) {
    ...
    if (result.cases.length !== manifest.cases.length) {
      throw new Error(`${result.language}: expected ${manifest.cases.length} mandatory cases, got ${result.cases.length}`);
    }
    for (const item of result.cases) {
      ...
      if (expected.expected.signature !== "not-applicable" || item.signature.status !== "not-applicable") {
...
if (import.meta.main) {
  ...
  console.log(JSON.stringify(compareAgentConformance(repositoryRoot, argumentValues("--result")), null, 2));
}
