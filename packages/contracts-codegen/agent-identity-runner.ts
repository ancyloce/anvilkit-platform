...
function allowedPurposes(repositoryRoot: string): Set<string> {
  const registry = JSON.parse(readFileSync(join(repositoryRoot, "contracts/agent/registries/registry-set.json"), "utf8")) as {
...
export function buildTypeScriptIdentityResult(repositoryRoot: string): ConformanceResult {
  ...
  const corpusPath = join(repositoryRoot, "contracts/agent/fixtures/canonical/identity-cases.json");
  ...
  const adapterSources = ["native-identity.ts", "strict-json.ts"]
