...
- `infra/k8s/agent-runtimes-config.example.yaml` — the non-secret configuration the
  Agent Runtime Units read: the `anvilkit-runtime` ConfigMap naming the governed
  control-plane origin, and the operator-distributed `anvilkit-task-credential-trust`
  snapshot (placeholder key material; the real snapshot comes from operator key
  distribution per ADR-006). Each unit's own deployment material — digest-pinned
  Deployment + HPA, and `identity.yaml` with its ServiceAccount and least-privilege
  NetworkPolicy — lives beside the unit in
  `services/agent-runtimes/agents/<unit>/deploy/`. Per-unit runtime-manifest
  ConfigMaps are generated by the release pipeline
  (`services/agent-service/cmd/runtime-release`; see
  `docs/runbooks/agent-runtime-release.md`), never hand-written. All of it is
  kubeconform-validated in CI, and `deploy.yml` promotes a unit by immutable image
  digest — pick the unit name for `service` and pass `image_digest`.
...
## Kernel test database (P0-Kernel WP0)

The Agent Service PostgreSQL proofs — persistence and migrations, the journal store, the
DBOS system database, and the cmd-level restart, custody, composition, and vertical-slice
integration tests — need a real PostgreSQL they may create and drop databases and schemas
in. `infra/docker-compose.kernel-test.yml` provides a disposable one, separate from the
export-worker stack above, managed through `scripts/test-postgres.sh`:

```bash
scripts/test-postgres.sh up        # start it and print POSTGRES_TEST_URL
scripts/test-postgres.sh status    # is it reachable?
scripts/test-postgres.sh down      # remove it (its data directory is a tmpfs)
```

`scripts/kernel-verify.sh` discovers a running one by itself, so the usual loop is
`scripts/test-postgres.sh up && scripts/kernel-verify.sh`. Without a database the
PostgreSQL proofs are reported **blocked** and the run fails: they never skip silently.
To use a different instance, export `POSTGRES_TEST_URL` (and `DBOS_TEST_URL` if the DBOS
system database lives elsewhere) — pointing either at anything but a disposable database
will lose data.
